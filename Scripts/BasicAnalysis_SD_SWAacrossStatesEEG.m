...
for n=1:n_WT
    ...
    for d=1:2
        for ld=1:2
            if ld==1
                ...
                if d==1
                    WT_swa(n,1:21600)=nanmean(Spectr(:,5:17),2)';
                    Wake_logical(W)=1;NR_logical(NR)=1;R_logical(R)=1;
                else
                    WT_swa(n,21601:43200)=nanmean(Spectr(:,5:17),2)';
    ...
    x_hours=0:4/3600:48;x_hours=x_hours(1:43200);
    subplot(n_WT,1,n)
    plot(x_hours,Wake_swa);
    hold on
    plot(x_hours,NR_swa);
    hold on
    plot(x_hours,R_swa);
    ...
    for d=1:2
        for ld=1:2
            if ld==1
                ...
                if d==1
                    HOM_swa(n,1:21600)=nanmean(Spectr(:,5:17),2)';
                    Wake_logical(W)=1;NR_logical(NR)=1;R_logical(R)=1;
                else
                    HOM_swa(n,21601:43200)=nanmean(Spectr(:,5:17),2)';
                    Wake_logical(W+21600)=1;NR_logical(NR+21600)=1;R_logical(R+21600)=1;
                end
