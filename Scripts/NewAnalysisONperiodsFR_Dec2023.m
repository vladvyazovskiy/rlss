    ...
    for anm=1:5
        ...
        if genotype==2 && anm==2 % No values for Me
            ...
            for phase=1%phase_scope % AU L, AU D 
                ...
                mousename=Names{anm};
                if phase==1
                    end_name='Newclustering';recorddate=[phase_name(1:end-1),'_L'];%%%recorddate=[phase_name_aud(1:end-1),'_L'];
                else
...
%     
                ...
                for j=1:length(StartON_indexes_allVS) 
                    ...
                    FR_ALL_ONperiods=[FR_ALL_ONperiods;FR_ONperiod];
 
                end
            end
            if genotype==1  
                %%%%FR_Allmice_WT{anm}=mean(FR_ALL_ONperiods,2,FR_Allmice_WT{anm});
                FR_Allmice_WT{anm}=mean(FR_ALL_ONperiods,2,'omitnan');
     
            else 
                FR_Allmice_Rlss{anm}=mean(FR_ALL_ONperiods,2,'omitnan');
