...
bin_size=10;
max_duration=2000;
xq=0:bin_size:max_duration; % Query points for the interpolation, so as to be able to average across channels and animals.
...
for an_WT=Included_WT
    ...
    f_BL_NR=cell(1,num_ch);x_BL_NR=cell(1,num_ch);Survival_BL_NR=NaN(num_ch,length(xq));
    f_BL_R=cell(1,num_ch);x_BL_R=cell(1,num_ch);Survival_BL_R=NaN(num_ch,length(xq));
    f_SD_NR=cell(1,num_ch);x_SD_NR=cell(1,num_ch);Survival_SD_NR=NaN(num_ch,length(xq));
    f_SD_R=cell(1,num_ch);x_SD_R=cell(1,num_ch);Survival_SD_R=NaN(num_ch,length(xq));
    Half_lives_BL_NR=NaN(1,num_ch);Half_lives_BL_R=NaN(1,num_ch);Half_lives_SD_NR=NaN(1,num_ch);Half_lives_SD_R=NaN(1,num_ch);
    %% BL NR
    for ch=1:num_ch
        
        Survival_BL_NR(ch,:) = calculate_survival(OP_durations_all_BL_NR{1,ch},bin_size,xq);
        [M, Half_lives_BL_NR(1,ch)]=min(abs(0.5-Survival_BL_NR(ch,:)));%xq(given_index)=given_index-1 so recording the index where the min occurs will be here considered equivalent to recording the abscissa of the half-life.
    end
    ...
    for ch=1:num_ch
        ...
        Survival_BL_R(ch,:) = calculate_survival(OP_durations_all_BL_R{1,ch},bin_size,xq);
        [M, Half_lives_BL_R(1,ch)]=min(abs(0.5-Survival_BL_R(ch,:)));
    end
    ...
    if an_WT ~= 4
        for ch=1:num_ch
            ...
            Survival_SD_NR(ch,:) = calculate_survival(OP_durations_all_SD_NR{1,ch},bin_size,xq);
            [M, Half_lives_SD_NR(1,ch)]=min(abs(0.5-Survival_SD_NR(ch,:)));
        end
        
        %% SD R
        for ch=1:num_ch

            Survival_SD_R(ch,:) = calculate_survival(OP_durations_all_SD_R{1,ch},bin_size,xq);
            [M, Half_lives_SD_R(1,ch)]=min(abs(0.5-Survival_SD_R(ch,:)));
        end
    end
    
    Average_Curve_All_BL_NR_WT(an_WT,:)=mean(Survival_BL_NR,'omitnan');
    Average_Curve_All_BL_R_WT(an_WT,:)=mean(Survival_BL_R,'omitnan');
    Average_Curve_All_SD_NR_WT(an_WT,:)=mean(Survival_SD_NR,'omitnan');
    Average_Curve_All_SD_R_WT(an_WT,:)=mean(Survival_SD_R,'omitnan');
...
for an_Rstl=Included_Rstl
    ...
    for ch=1:num_ch
        ...
        Survival_BL_NR(ch,:) = calculate_survival(OP_durations_all_BL_NR{1,ch},bin_size,xq);
        [M, Half_lives_BL_NR(1,ch)]=min(abs(0.5-Survival_BL_NR(ch,:)));
    end
    ...
    Average_Curve_All_BL_NR_Rstl(an_Rstl,:)=mean(Survival_BL_NR,'omitnan');
    Average_Curve_All_BL_R_Rstl(an_Rstl,:)=mean(Survival_BL_R,'omitnan');
    Average_Curve_All_SD_NR_Rstl(an_Rstl,:)=mean(Survival_SD_NR,'omitnan');
    Average_Curve_All_SD_R_Rstl(an_Rstl,:)=mean(Survival_SD_R,'omitnan');
...
figure(1)
subplot(2,2,1)
plot(xq/SR_pNE,mean(Average_Curve_All_BL_NR_WT,'omitnan'),'Color',[0.1 0.2 0.9],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_BL_NR_Rstl,'omitnan'),'Color',[0.9 0.2 0.1],'LineWidth',1.5)
legend('WT', 'Rstl')
% hold on
...
%xlim([0 1000])
set(gca, 'YScale', 'log')
ylabel('Fraction of OFF periods')
xlabel('OFF period duration (sec)')

subplot(2,2,2)
plot(xq/SR_pNE,mean(Average_Curve_All_SD_NR_WT,'omitnan'),'Color',[0.1 0.2 0.9],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_NR_Rstl,'omitnan'),'Color',[0.9 0.2 0.1],'LineWidth',1.5)
legend('WT', 'Rstl')
% hold on
...
subplot(2,2,3)
plot(xq/SR_pNE,mean(Average_Curve_All_BL_R_WT,'omitnan'),'Color',[0.1 0.2 0.9],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_BL_R_Rstl,'omitnan'),'Color',[0.9 0.2 0.1],'LineWidth',1.5)
legend('WT', 'Rstl')
% hold on
...
subplot(2,2,4)
plot(xq/SR_pNE,mean(Average_Curve_All_SD_R_WT,'omitnan'),'Color',[0.1 0.2 0.9],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_R_Rstl,'omitnan'),'Color',[0.9 0.2 0.1],'LineWidth',1.5)
legend('WT', 'Rstl')
% hold on
...
figure(2)
subplot(2,2,1)
plot(xq/SR_pNE,mean(Average_Curve_All_BL_NR_WT,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_NR_WT,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('BL', 'SD')
% hold on
...
subplot(2,2,2)
plot(xq/SR_pNE,mean(Average_Curve_All_BL_R_WT,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_R_WT,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('BL', 'SD')
% hold on
...
subplot(2,2,3)
plot(xq/SR_pNE,mean(Average_Curve_All_BL_NR_Rstl,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_NR_Rstl,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('BL', 'SD')
% hold on
...
subplot(2,2,4)
plot(xq/SR_pNE,mean(Average_Curve_All_BL_R_Rstl,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_R_Rstl,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('BL', 'SD')
% hold on
...
plot(xq/SR_pNE,mean(Average_Curve_All_BL_NR_WT,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_BL_R_WT,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('NR', 'R')
% hold on
% h1=plot(xq,1-mean(Average_Curve_All_BL_NR_WT,'omitnan')+std(Average_Curve_All_BL_NR_WT,'omitnan')/sqrt(length(Included_WT)),'.','Color',[0.5 0.5 0.5],'MarkerSize',0.2);
% h1.Annotation.LegendInformation.IconDisplayStyle = 'off';
...
subplot(2,2,2)
plot(xq/SR_pNE,mean(Average_Curve_All_SD_NR_WT,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_R_WT,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('NR', 'R')
% hold on
...
plot(xq/SR_pNE,mean(Average_Curve_All_BL_NR_Rstl,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_BL_R_Rstl,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('NR', 'R')
% hold on
% h1=plot(xq,1-mean(Average_Curve_All_BL_NR_Rstl,'omitnan')+std(Average_Curve_All_BL_NR_Rstl,'omitnan')/sqrt(length(Included_Rstl)),'.','Color',[0.5 0.5 0.5],'MarkerSize',0.2);
% h1.Annotation.LegendInformation.IconDisplayStyle = 'off';
...
subplot(2,2,4)
plot(xq/SR_pNE,mean(Average_Curve_All_SD_NR_Rstl,'omitnan'),'Color',[0.5 0.5 0.5],'LineWidth',1.5)
hold on
plot(xq/SR_pNE,mean(Average_Curve_All_SD_R_Rstl,'omitnan'),'Color',[0.6 0.2 0.6],'LineWidth',1.5)
legend('NR', 'R')
% hold on
...
%%%% Function to calculate the survival curve

function survival_matrix=calculate_survival(OP_durations_matrix,bin_size,xq)

survival_matrix=zeros(1,length(xq));
for i=1:length(xq)
    survival_matrix(1,i)=(sum(OP_durations_matrix(OP_durations_matrix~=0)>(i-1)*bin_size)/length(OP_durations_matrix(OP_durations_matrix~=0)));
end
